...
Basic templates **`${}`**, adicionados na versão 4.1+
```ts
// Template literal types
 ...
 **Tipos nativos (built-in):** São funções nativas já disponíveis para manipulação de dados, basta usar.

PS: este mesmo arquivo tem as funções `Pick, Partial, Record, Promise, Date, etc, ...`
```ts
...
const a: T1 = "HELLO" // não aceita hello como valor inicializador!
